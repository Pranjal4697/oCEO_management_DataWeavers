    ...
    if "roll_number" in session:
        ...
        if request.method == 'POST':
            if request.form['submit_button'] == "submit_timecard":
                ...
                sql = f"INSERT INTO time_card (roll_number, job_id, month, year, hours_worked, work_description, faculty_approval,oceo_coordinator_approval, payment_status) VALUES ({roll_number}, {job_id}, '{month}', {year}, {hours_worked}, '{work_description}','pending', 'pending','pending');"
                cursor.execute(sql)
                db.connection.commit()
                cursor.close()  
